  ...
  try {
    ...
    if (rows[0].count === 0) {
      await db.execute(`
        ...
        SELECT WalkRequests.request_id, Users.user_id, Dogs.owner_id, 5, 'Bob has been lovely to our pup. He always shows up with a treat in his hat!' FROM WalkRequests INNER JOIN Users ON Dogs.owner_id = Users.user_id LIMIT 1
      `);
      await db.execute(`
        INSERT INTO WalkRequests (dog_id, requested_time, duration_minutes, location, status)
        SELECT dog_id, '2025-06-10 09:30:00', 45, 'Beachside Ave', 'accepted' FROM Dogs WHERE name = 'Bella' LIMIT 1
