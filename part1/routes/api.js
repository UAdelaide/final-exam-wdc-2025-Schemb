...
router.get('/walkers/summary', async (req, res) => {
  try {
    const [openWalkRequests] = await db.execute(`SELECT Users.username AS walker_username, COUNT(WalkRatings.rating_id) AS total_ratings, AVG(WalkRatings.rating) AS average_rating, COUNT(WalkApplications.application_id) AS completed_walks FROM Users LEFT JOIN WalkRatings ON Users.user_id = WalkRatings.walker_id LEFT JOIN WalkApplications ON Users.user_id = WalkApplications.walker_id GROUP BY Users.username`);
    res.json(openWalkRequests);
  } catch (err) {
