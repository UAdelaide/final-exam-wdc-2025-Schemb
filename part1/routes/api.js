...
router.get('/walkers/summary', async (req, res) => {
  try {
    const [openWalkRequests] = await db.execute(`SELECT Users.username AS walker_username, total_ratings, average_rating, completed_walks FROM Users INNER JOIN WalkRatings ON Users.user_id AS curr_user_id = WalkRatings.walker_id WHERE total_ratings = (SELECT COUNT(*) FROM WalkRatings WHERE walker_id = )`);
    res.json(openWalkRequests);
  } catch (err) {
